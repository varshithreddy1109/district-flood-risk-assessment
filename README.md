...
The system processes historical flood data to classify districts into four risk quartiles: **Low**, **Moderate**, **High**, and **Very High**. It provides an interactive Streamlit dashboard that visualizes the classified risk level, model probability, historical indicators, and feature contributions for any selected district in India.

## Key Features

* **Historical Risk Classification**: Classifies 731 usable Indian district records into historical risk quartiles.
* **Interactive Dashboard**: A clean, professional Streamlit interface for exploring district-level data.
* **Contextual Rainfall**: Displays current rainfall context alongside historical indicators. Current rainfall is not used as an ML feature because the model is designed to classify historical flood-risk patterns.
...
The system uses SHAP `TreeExplainer` to calculate global feature importance and local feature contributions for individual districts. This allows users to see which historical features contributed most to a specific model classification. These contributions represent model behavior, not causal relationships.

Permutation importance is also used on held-out data to evaluate the predictive usefulness of individual features.
...
* `outputs/`: Directory containing generated model evaluation and explainability artifacts.
...
* **HydroSense Lab, IIT Delhi / Zenodo**: India Flood Inventory and DFSI-related flood datasets. Used for historical modeling and DFSI target derivation.
  ...
  * Source: https://zenodo.org/records/16994648
...
* **India Meteorological Department (IMD) / NWDP**: Rainfall data accessed through the National Water Data Portal. Used **only** as contextual current rainfall information in the dashboard, and **NOT** as an input to the machine-learning model.
  ...
  * Source: https://www.nwdp.nwic.gov.in/en/dataset/rainfall-daily-imd/resource/8752174f-1d17-4aaf-8058-2eb396f50157
...
* **Current Rainfall is Contextual Only**: Current rainfall information displayed in the dashboard is not used by the machine-learning model.
